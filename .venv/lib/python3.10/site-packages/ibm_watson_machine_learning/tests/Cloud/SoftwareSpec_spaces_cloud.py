...
class TestSwSpec(unittest.TestCase):
    ...
    def test_05_details_with_state(self):
        details = self.client.software_specifications.get_details(state_info=True)
        states = [detail['state'] for detail in details['resources']]
        self.assertEqual(len(states), len(details['resources']))

    def test_06_delete_sw_spec(self):
